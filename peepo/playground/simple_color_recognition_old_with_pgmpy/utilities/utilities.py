...
class Utilities(object):
    def __init__(self, file):
        ...
        self.header = ''
        self.dictionary = []
        self.summary = {'raw_cpd': {}, 'pom_cpd': {}, 'cardinality': {}, 'edges': {}, 'parents': {},
    ...
    def get_edges(self):
        ...
        edges = dict()
        [edges[str(t[0])].append(str(t[1])) if t[0] in list(edges.keys()) else edges.update({str(t[0]): [str(t[1])]})
         for t in edg]
        return edges
    ...
    def get_nodes_and_attributes(self):
        ...
        nod_and_attributes = []
        [nod_and_attributes.append((str(node), {'cardinality': int(self.pgmpy_object.get_cardinality(node)),
                                                'cpd': self.pgmpy_object.get_cpds(node).values.astype(float)})) for
    ...
    def translate_digraph_to_pomegranate(self, digraf):
        ...
        :rtype :networkx:adress
        """
        self.pomegranate, self.pom_nodes, self.summary, x, y = self.get_pomegranate_network(from_object=True,
                                                                                            digraph=digraf)
    ...
    def get_pomegranate_network(self, from_object=False, digraph=None):
        ...
        for i, node in enumerate(nw_nodes):
            ...
            if len(parent) != 0:
                continue
            cpd_parents = []
            cpd_p = self.translate_cpd_to_pomegranate('RONS', node_name, cardinality, cpd, cpd_parents,
                                                      parent_cardinality, child)
        ...
        for i, node in enumerate(nw_nodes):
            ...
            cpd_p = self.translate_cpd_to_pomegranate('LANS', node_name, cardinality, cpd, cpd_parents,
                                                      parent_cardinality, child)
            self.summary['pom_cpd'].update({node_name: cpd_p})
        ...
        for i, node in enumerate(nw_nodes):
            ...
            cpd_p = self.translate_cpd_to_pomegranate('LENS', node_name, cardinality, cpd, cpd_parents,
                                                      parent_cardinality, child)
            self.summary['pom_cpd'].update({node_name: cpd_p})
        ...
        for key in data['Nodes'].keys():
            for secondkey in data['Nodes'][key].keys():
                for c, n in enumerate(data['Nodes'][key][secondkey]):
                    ...
                    self.networkx_object.add_node(node, {'cardinality': n[1], 'cpd': [], 'parents': [],
                                                         'parents_cardinality': [], 'childs': []})
                    self.dictionary.append((n[0], node))
                    cardinality.update({node: n[
                        1]})  # this contains the cardinality of each node with the node name as dictionary entry
                    parents.update({node: []})
    ...
    def get_empty_canvas(self):
        ...
        '''       - feeding the data'''
        data['Nodes'] = {'RONS': {'RON_BEN': bens, 'RON_': mems}, 'LANS': {'LANS': lans},
                         'LENS': {'LEN_MOTOR': motors, 'LEN_WORLD': world}}
        data['Edges'] = edges
        data['CPDs'] = cpds
        return data
